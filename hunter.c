...
void decideHunterMove(HunterView gameState)
{
	...
	if(giveMeTheRound(gameState) == 0){
        if(whoAmI(gameState) ==  PLAYER_LORD_GODALMING){
            registerBestPlay(idToName(MANCHESTER),"I'm on holiday in Manchester");
        } else if(whoAmI(gameState) ==  PLAYER_DR_SEWARD){
            registerBestPlay(idToName(ROME),"I'm on holiday in Rome");
        } else if(whoAmI(gameState) ==  PLAYER_VAN_HELSING){
            registerBestPlay(idToName(MADRID),"I'm on holiday in Madrid");
        } else if(whoAmI(gameState) ==  PLAYER_MINA_HARKER){
            registerBestPlay(idToName(SZEGED),"I'm on holiday in Szeged");
        } 
    } else {
